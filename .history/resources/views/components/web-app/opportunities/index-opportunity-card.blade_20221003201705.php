...
@php
    ...
    //process opportunity seek after validating that the collection is not empty
    if (!$opportunity->opportunity_seeking->isEmpty()) {

        //1. declare image path
        $opportunity_gap = [];

        //2 loop through image collection
        foreach ($opportunity->opportunity_seeking as $seeking) {

            //dd($image_details->image_path);
            $opportunity_gap[] = '<a href="/opportunities?seeking='.$seeking['seeking_name'].'"><span class="finance-details">'.Str::ucfirst($seeking['seeking_name']).'</span></a>';
        }

        //3. implode
        $opportunity_seek = implode('<span class="funding-separator current-breadcrumbs">&nbsp;&nbsp;|&nbsp;&nbsp;</span>', $opportunity_gap);

    }else{

        $opportunity_seek = '<span class="finance-details current-breadcrumbs">Seeking details unavailable</span>';
        ...
        <article class="geodir-category-listing fl-wrap">
            <div class="geodir-category-img">
                ...
                @if ($current_route != 'web-app.dashboard.show-user-profile')
                    <div class="listing-avatar">
                        ...
                        @endif
                        <span class="avatar-tooltip" style="min-width: 250px; margin-top: -3px !important;">Added By
                            <a href="{{ $opportunity_user_path }}">
                                <strong>
                ...
                <div class="geodir-category-opt">
                    <div class="listing-rating-count-wrap">
                        ...
                        <div class="reviews-count">
                            {{ $opportunity->views_count }}&nbsp;&nbsp;view{{ $opportunity->views_count > 1 ? 's' : '' }}&nbsp;&nbsp;|&nbsp;&nbsp;<a href="/opportunities/{{ $opportunity->id }}-{{ Str::slug($opportunity->title) }}#opportunity_reviews" class="normal_link"><span>{{ $total_reviews }}&nbsp;&nbsp;review{{ $total_reviews > 1 ? 's' : '' }}</span></a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="geodir-category-content fl-wrap title-sin_item" style="margin-top: 10px !important;">
                ...
                <div class="geodir-category-text fl-wrap">
                    ...
                    <div class="facilities-list fl-wrap" style="margin-top: 24px !important;">
                        <div class="facilities-list-title">Opportunity seeking:</div><br>
                        <ul class="no-list-style" style="text-align: left !important; padding-top: 10px !important;">
                            {!! $opportunity_seek !!}
                        </ul>
                    </div>
                </div>
                <div class="geodir-category-footer fl-wrap" style="min-height: 65px;">
                    <a href="/opportunities?interest=&category_id={{ $category_details->id }}&country_id=All+Locations" class="listing-item-category-wrap" style="padding-top: 3px !important;">
                        <div class="listing-item-category color2-bg">
                            <i class="fal fa-list-alt"></i>
                        </div>
                        <span class='opportunity_category_name'>{{ $category_details->name }}</span>
                    </a>
