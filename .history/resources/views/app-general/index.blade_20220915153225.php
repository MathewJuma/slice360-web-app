...
<x-app-general.main-web-app-layout :all_countries='$all_countries' :all_categories='$all_categories' :user_details='[]'>
    ...
    <section class="parallax-section small-par" data-scrollax-parent="true">
        @php
            ...
            $last_day_this_month  = date('m-t-Y 23:59:59'); //last date of the current month

            foreach ($all_opportunities as $opportunitiy) {
                dd(date('d-m-Y H:i:s', strtotime($opportunitiy->created_at)));
                if($opportunitiy->created_at >= $first_day_this_month &&  $opportunitiy->created_at <= $last_day_this_month){
                    $opportunity_count++;
                }
            }
        @endphp
