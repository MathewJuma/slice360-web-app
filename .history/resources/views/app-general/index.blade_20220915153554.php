...
<x-app-general.main-web-app-layout :all_countries='$all_countries' :all_categories='$all_categories' :user_details='[]'>
    ...
    <section class="parallax-section small-par" data-scrollax-parent="true">
        @php
            ...
            $opportunity_count = 0;
            $total_full_funding = 0;

            $first_day_this_month = strtotime(date('m-01-Y 00:00:00')); // hard-coded '01' for first day
            $last_day_this_month  = strtotime(date('m-t-Y 23:59:59')); //last date of the current month

            foreach ($all_opportunities as $opportunitiy) {

                //created_at
                $created_at = strtotime(date('d-m-Y H:i:s', strtotime($opportunitiy->created_at)));
                dd($first_day_this_month);

                if($created_at >= $first_day_this_month &&  $created_at <= $last_day_this_month){
                    $opportunity_count++;
                }
            }
        @endphp
