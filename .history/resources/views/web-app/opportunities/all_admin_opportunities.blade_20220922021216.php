...
<x-web-app.dashboard.dashboard-web-app-layout :all_countries='$all_countries' :all_categories='$all_categories' :all_opportunities='$all_opportunities' :user_details='$user_details'>
    ...
    <table class="table table-bordered styled-table">
        ...
        <tbody>
            ...
            {{ dd($all_opportunities->firstItem()) }}

            {{-- loop through each opportunity --}}
            @php

                $table_row = '';
                foreach ($all_opportunities as $key => $opportunity) {

                    //status
                    $status = $opportunity->status == 1 ? 'Active' : 'Inactive';

                    $table_row .= ' <tr style>
                                        <td style="text-align: right !important; padding-right: 10px !important;">'. $opportunity->firstItem() + $key .'.</td>
