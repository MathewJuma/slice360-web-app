            ...
            $('#news_subscription_form').click(function() {
                ...
                $(news_subscription_form).validate({
                    ...
                    submitHandler: function() {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('web-app.blog.news-subscription') }}",
                            data: form_data,
                            cache: false,
                            //dataType: "JSON",
                            //processData: false,
                            ...
                            success: function(response) {
                                ...
                                if($.isEmptyObject(response.exists)){
                                    ...
                                    if(response.success){
                                        ...
                                        //clear all the data
                                        $('#news_subscription_form').each(function() {
                                            this.reset();
                                        });

                                        $('.jquery_nofications').fadeIn();
                                        $('.jquery_nofications').html(
                                            '<div class="add-list color-bg" id="jquery_nofications_wrapper">Thankyou for subscription to our newsletter</div>'
                                        );
                                    ...
                                    if(response.exists){
                                        ...
                                        $('.jquery_nofications').fadeIn();
                                        $('.jquery_nofications').html(
                                            '<div class="add-list color-bg" id="jquery_nofications_wrapper" style="color: #dc3545 !important; !important;">You have already subscribed to our newsletter. Thankyou</div>'
                                        );
