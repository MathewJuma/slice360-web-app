...
class EloquentViewableServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        if ($this->app->runningInConsole()) {
            ...
            if (! class_exists('CreateSlice360PageViewsTable')) {
                $timestamp = date('Y_m_d_His', time());

                $this->publishes([
                    __DIR__.'/../migrations/create_slice360_page_views_table.php.stub' => database_path("/migrations/{$timestamp}_create_slice360_page_views_table.php"),
                ], 'migrations');
            }
        }
    }
