 ...
 */
class WinkPost extends AbstractWinkModel implements Viewable
{
    ...
    public function scopeTitle($query, array $filters)
    {
        //dd($filters);
        $search_blog = ['search_blog'];

        //process only if filters search is not empty
        if ($filters['search'] ?? false) {
            $query->where('title', 'like', '%'.$filters['search'].'%') //select where title matches filters
                ->orWhere('description', 'like', '%'.$filters['search'].'%') //select where description matches filters
                ->orWhere('tags', 'like', '%'.$filters['search'].'%'); //select where tags matches filters
        }
    }
