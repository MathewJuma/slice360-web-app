...
class WinkPost extends AbstractWinkModel implements Viewable
{
    ...
    public function scopeTitle($query, string $search_filetr)
    {
        //dd($filters);

        //process only if filters search is not empty
        if ($title ?? false) {
            return $query->where('title', 'like', '%'.$title.'%') //select where title matches filters
                ->orWhere('excerpt', 'like', '%'.$title.'%'); //select where excerpt matches filters
        }
    }
