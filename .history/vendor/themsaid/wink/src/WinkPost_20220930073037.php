...
class WinkPost extends AbstractWinkModel implements Viewable
{
    ...
    public function scopeTitle($query, string $filters)
    {
        //dd($filters);

        //process only if filters search is not empty
        if ($filters ?? false) {
            return $query->where('title', 'like', '%'.$filters.'%') //select where title matches filters
                ->orWhere('excerpt', 'like', '%'.$filters.'%'); //select where excerpt matches filters
        }
    }
