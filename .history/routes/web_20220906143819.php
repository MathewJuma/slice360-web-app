...
Route::controller(DashboardController::class)->name('web-app.dashboard.')->group(function () {
    Route::get('/dashboard/index', 'mainDashboard')->name('main-dashboard');
    Route::get('/dashboard/user_profile/{user}/edit', 'editUserProfile')->name('main-dashboard')->middleware('auth');
    Route::patch('/dashboard/user_profile/{user}/', 'updateUserProfile')->name('update-opportunity')->middleware('auth');
});
