...
class OpportunitiesController extends Controller
{
    ...
    public function showOpportunity(Request $request, Opportunity $opportunity)
    {
        ...
        //A. before showing an opportunity, set cookies to track viewers
        ...
        //B. Validate how cookie has been set to determine if view count is valid
        if(Cookie::get($cookie_name) == ''){//check if cookie is set

            $cookie = cookie($cookie_name, '1', 60);//set the cookie
            $opportunity->incrementViewCount();//count the view
            //return response()->view('posts.show',['post' => $post])->withCookie($cookie);//store the cookie

        } else {

            //return  view('posts.show',['post' => $post]);//this view is not counted
