...
class OpportunitiesController extends Controller
{
    ...
    public function adminFetchOpportunities(Request $request)
    {
        ...
        if ($request->filled('search')) {
            ...
            $all_opportunities = Opportunity::search($request->search)
                                ->query(fn ($query) => $query->with(['opportunity_user', 'opportunity_country', 'opportunity_category', 'opportunity_banner_images', 'opportunity_other_images']))
                                ->paginate(10, 'slice360-opportunities'); // search by value
            //dd($all_opportunities);

        } else {

            $all_opportunities = Opportunity::with(['opportunity_user', 'opportunity_country', 'opportunity_category', 'opportunity_banner_images', 'opportunity_other_images'])->paginate(10, '*', 'slice360-opportunities');
        ...
        return view('web-app.opportunities.all_admin_opportunities', compact(['all_countries', 'all_categories', 'all_opportunities', 'user_details']));
