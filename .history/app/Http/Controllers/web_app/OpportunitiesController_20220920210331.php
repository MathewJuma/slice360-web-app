...
class OpportunitiesController extends Controller
{
    ...
    public function listOpportunities()
    {
        ...
        $all_opportunities = Opportunity::with(['opportunity_user', 'opportunity_country', 'opportunity_category', 'opportunity_banner_images', 'opportunity_other_images'])->whereRaw('funding_status != "funding closed"')->latest()->filter(request(['tag', 'interest', 'country_id', 'category_id']))->paginate(9, '*', 'slice360-opportunities');
    ...
    public function adminOpportunitiesPage()
    {
        //1. all countries and categories
        $all_countries = $this->all_countries;
        $all_categories = $this->all_categories;
        ...
        return view('web-app.opportunities.all_admin_opportunities', compact(['all_countries', 'all_categories', 'user_details']));

    }


    //this function is used by Admins to fetch oppotunities
    public function adminFetchOpportunities(Request $request)
    {

        $all_opportunities = Opportunity::with(['opportunity_user', 'opportunity_country', 'opportunity_category', 'opportunity_banner_images', 'opportunity_other_images'])->whereRaw('funding_status != "funding closed"')->latest()->get();

        //check if request is ajax
        if($request->ajax()){

            //laod data into datatebale
            return DataTables::of(all_opportunities)
                    ->addColumn('edit_action', function ($row) {
                        return '
                            <a href="javascript:void(0)" class="btn btn-sm btn-primary editButton" data-id="'.$row->id.'">Edit</a>
                            <a href="javascript:void(0)" class="btn btn-sm btn-danger deleteButton" data-id="'.$row->id.'">Delete</a>
                        ';
                    })
                    ->rawColumns(['edit_action'])
                    ->addIndexColumn()
                    ->make(true);
