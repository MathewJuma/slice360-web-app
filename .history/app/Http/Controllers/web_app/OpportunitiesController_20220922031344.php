...
class OpportunitiesController extends Controller
{
    ...
    public function adminFetchOpportunities(Request $request)
    {
        ...
        if ($request->filled('search')) {
            $students = Student::search($request->search)->get(); // search by value
        } else {
            $students = Student::get()->take('10'); // list 10 rows
        }
        $all_opportunities = Opportunity::with(['opportunity_user', 'opportunity_country', 'opportunity_category', 'opportunity_banner_images', 'opportunity_other_images'])->paginate(10, '*', 'slice360-opportunities')->onEachSide(5);
