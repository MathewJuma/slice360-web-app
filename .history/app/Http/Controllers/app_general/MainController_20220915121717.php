...
class MainController extends Controller
{
    ...
    public function home()
    {
        ...
        $new_weekly_opportunities = count(Slice360Visitor::select('*')->whereMonth('created_at', Carbon::now()->month)->get());
        $new_monthly_opportunities = count(Opportunity::select('*')->whereMonth('created_at', Carbon::now()->month)->get());

        return view('app-general.index', compact(['all_countries', 'all_categories', 'all_opportunities', 'month_opportunities','testimonials', 'monthly_site_visitors']));
    }
