...
class MainController extends Controller
{
    ...
    public function home()
    {
        ...
        $all_opportunities = Opportunity::with(['opportunity_user', 'opportunity_country', 'opportunity_category', 'opportunity_banner_images', 'opportunity_other_images'])
                                            ...
                                            ->filterOpportunities(request(['tag', 'interest', 'country_id', 'category_id']))->get();

        //opportunities for statistics
        $statistics_opportunities = Opportunity::with(['opportunity_user', 'opportunity_country', 'opportunity_category', 'opportunity_seeking', 'opportunity_banner_images', 'opportunity_other_images'])
                                            ->whereRaw('funding_status = "funding closed"')
                                            ->orderBy('id', 'DESC')->orderBy('created_at', 'DESC')->latest()
                                            ->filterOpportunities(request(['tag', 'seeking', 'interest', 'country_id', 'category_id']))->get();
        ...
        $popular_opportunities = (Opportunity::with(['opportunity_user', 'opportunity_country', 'opportunity_category', 'opportunity_banner_images', 'opportunity_other_images'])
                                            ...
                                            ->orderBy('id', 'DESC')->orderBy('created_at', 'DESC')->latest()
                                            ->take(50)->get())->where('views_count', '!=', '0')->paginate(8, '', '', 'popular-opportunities');
