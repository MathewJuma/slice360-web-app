...
class MainController extends Controller
{
    ...
    public function home()
    {
        ...
        $all_opportunities = Opportunity::with(['opportunity_user', 'opportunity_country', 'opportunity_category', 'opportunity_banner_images', 'opportunity_other_images'])->latest()->filter(request(['tag', 'interest', 'country_id', 'category_id']))->all();
        $popular_opportunities = Opportunity::with(['opportunity_user', 'opportunity_country', 'opportunity_category', 'opportunity_banner_images', 'opportunity_other_images'])->orderByViews('desc', $views_period)->orderByUniqueViews('desc', $views_period)->latest()->filter(request(['tag', 'interest', 'country_id', 'category_id']))->paginate(8);
        ...
        //dd($sum_funded_opportunities);

        return view('app-general.index', compact(['all_countries', 'all_categories', 'all_opportunities', 'testimonials', 'new_monthly_opportunities', 'new_monthly_visitors', 'fully_funded_opportunities', 'value_funded_opportunities']));
    }
